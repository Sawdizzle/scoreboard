...
- **Web Analytics** is Vercel's, added as one `<script defer src="/_vercel/insights/script.js">` on `index`, `/control` and `/recap`. It is served from this origin, so there is still no runtime CDN, no package and no build step — `npm i @vercel/analytics` is the framework path and would add a `package.json` and `node_modules` to a project that has neither. **`/overlay` is deliberately left out**: it is an OBS browser source that sits behind a camera for hours, and it is not a visit worth counting.
