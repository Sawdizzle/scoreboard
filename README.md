...
- **Situation bar** — inning, the count as dots (three balls, two strikes, two outs) and a mini diamond, live on every commit including rows from another device. Tapping it opens the **Situation sheet**, which holds every correction: the bases as a diamond you tap, Advance all / Clear, steppers for balls, strikes, outs and inning, Flip top/bottom, End half-inning, ◂ Previous batter / Next batter ▸, pitch count, runs/hits/errors folded away, Clear lineup under Lineups, and 🏁 End game at the foot. Every edit goes through the same `commit()` path, so all of it stays undoable — except the roster writes, which have no undo and say so.
- **The pad** — BALL · STRIKE, FOUL · OUT, RUN · RUNNERS, then one bar reading `1B | 2B | 3B | HR | HBP | E`. HR, a fourth ball, a third strike and a ball in play open sheets; the rest are one tap. Next batter left the pad for the Situation sheet: it is a correction, not a result.
- **Bottom row** — `↶ Undo`, `🎞️ Replay`, `🎥 Cameras`, `🧢 Field`, `🎬 On Air`, `⚙ Setup`, fixed, icon over label. Field is baseball-only and Cameras appears once OBS reports scenes, so most nights it is five. Cameras shows once OBS reports scenes and opens a sheet of scene cuts. Undo is what you reach for when something has already gone wrong, so it never scrolls away. Replay and On Air were each three or four taps deep in the drawer, which is too slow for a play that has just happened. A card that is up shows as a red chip in the header, and its ✕ takes it down. `End ½` moved into the Situation sheet — the third out already ends the half.
...
**🎞️ Replay** on the bottom bar saves the OBS replay buffer, so a great play is on disk before anyone asks for it. **Auto-clip big plays** (opt-in per game) does it for you on a home run, walk-off, double play, big play, touchdown, or goal — the buffer is retroactive, so the clip covers the play you just watched while both thumbs were still on the scoring pad.
...
An **OBS controls** switch at the top of the drawer's **OBS** tab hides `📡 Stream & record`, `🎥 Cameras`, `🎞️ Replay` on the bottom bar, and the auto-clip row. Scoring, cards, takeovers, moments, sound, sponsors, the overlay link and the recap are untouched — the overlay is a plain web page, so it works as a browser source in any streaming software or on a laptop wired to a screen.
...
| **Basic** | …plus 🎞️ Replay's clip and auto-clip |
...
- **New game:** choose sport, scorebug style and a start time (the start time drives the Starting Soon countdown and the order of the lobby's upcoming list).
...
- Supabase project `PickEm` (`yeykyutsbeqjcgdxlucn`), schema **`scoreboard`** — tables `games`, `events`, `plays`, `presets`, `rosters`, `teams`, `signup_attempts`; RPCs `apply_event`, `undo`, `server_now`, `save_roster`, `get_roster`, `overlay_token`, `rotate_overlay_token`, `get_plays`, `set_obs_scenes`, `set_obs_status`, `ack_replay`; edge function `signup`. The three that hold anything private — `rosters`, `plays` and `events` — have no public policy and are reached only through `SECURITY DEFINER` functions or as the owner.
...
6. **Page permissions: "Basic access to OBS"** — required for 🎞️ Replay to save the replay buffer. Leave it at the default if you don't want the overlay touching OBS.
...
- **Desktop** — same two columns, wider from 1280px, with hover feedback gated behind `(hover: hover)` so a tap never leaves a stuck highlight on touch. Keyboard scoring: `B S F O` (ball, strike, foul, out) · `1 2 3 H` (the hit bar) · `R` run · `E` error · `A` advance all · `C` clear bases · `N` runners · `U` undo — paused while a sheet is open, so a key never scores something the scrim is hiding.
