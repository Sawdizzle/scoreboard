...
- **Speed Insights** is Vercel's, added as one `<script defer src="/_vercel/speed-insights/script.js">` on `index`, `/control` and `/recap`. It is served from this origin, so there is still no runtime CDN, no package and no build step — `npm i @vercel/speed-insights` (or `@vercel/analytics`) is the framework path and would add a `package.json` and `node_modules` to a project that has neither. Web Analytics is a different product and is currently off; its `/_vercel/insights/script.js` 404s, so its tag is not in the pages. **`/overlay` is deliberately left out** of both: it is an OBS browser source that sits behind a camera for hours, and its load timings say nothing about anyone's visit.
